...
#
# Ralf Peine, Sun Dec 13 14:29:16 2020
#
#------------------------------------------------------------------------------
...
sub T400_auto_report {
    ...
    is (Report::Porf::Framework::auto_report(
            ...
            -file => $sfh),
        1, "Auto Report gives out single row (hrefs)");
    ...
    is (Report::Porf::Framework::auto_report(
            $data_href_rows,
            -file     => $sfh),
        12, "Auto Report gives out all 12 rows (hrefs)");
    ...
    is (Report::Porf::Framework::auto_report(
            ...
            -max_rows => 4,
            -file     => $sfh),
        4, "Auto Report -max_rows == 4 (hrefs)");
    ...
    is (Report::Porf::Framework::auto_report(
            ...
            -max_rows => 4,
            -file     => $sfh),
        4, "Auto Report -columns  => [] -max_rows == 4 (hrefs)");
    ...
    is (Report::Porf::Framework::auto_report(
            ...
            -max_rows => 4,
	    -file => $sfh),
        4, "Auto Report -columns  => [Geburtstag Vorname Nachname] -max_rows == 4 (hrefs)");

    # --- Test -------------------------------------------------------  
    is (Report::Porf::Framework::auto_report($data_href_rows),
        10, "Auto Report gives out max 10 rows without file handle (hrefs)");

    # diag ('Auto Report complete.');

    # --- Test -------------------------------------------------------  
    is (Report::Porf::Framework::auto_report(
            $data_arref_many_rows,
            -file     => $sfh),
        11, "Auto Report gives out all 11 rows (arrefs)");
    
    # --- Test -------------------------------------------------------  
    is (Report::Porf::Framework::auto_report(
            $data_arref_many_cols,
            -columns => [],
	    -file => $sfh
        ),
        3, "Auto Report array refs with empty -columns");
    ...
    is (Report::Porf::Framework::auto_report(
        ...
        3, "Auto Report array refs -columns 0 1 2 3");
    
    # --- Test -------------------------------------------------------  
    is (Report::Porf::Framework::auto_report(
            $data_arref_many_cols,
            -columns => [qw(Nr. A B C D E F)],
	    -file => $sfh
        ),
        3, "Auto Report array refs -columns Nr. A B C D E F");

    # --- Test -------------------------------------------------------  
    is (Report::Porf::Framework::auto_report(
            $data_arref_many_cols,
            -columns => [qw(Nr. A B -undef -undef E F G)],
	    -file => $sfh
        ),
        3, "Auto Report array refs -columns Nr. A B E F G");

    # --- Test -------------------------------------------------------  
    # --- Test: call ---
    my $report_configuration = Report::Porf::Framework::create_auto_report_configuration($data_href_rows);
    ...
    # --- Test: call ---
    $report_configuration = Report::Porf::Framework::create_auto_report_configuration($data_href_rows, -1);
    ...
    # --- Test: call ---
    $report_configuration = Report::Porf::Framework::report_configuration_as_string($data_href_rows);
